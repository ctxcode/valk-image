...
pure Valk. Reads and writes PNG, JPEG, WebP (lossy and lossless), GIF and BMP; resizes,
...
| `img.write(path)` | Encodes by extension (`.png`, `.jpg`/`.jpeg`, `.webp`, `.gif`, `.bmp`) and writes |
...
| `img.encode_webp(quality (75))` | Lossy WebP bytes; quality 0-100, alpha kept exactly |
...
The PNG, JPEG and BMP encoders have an `encode_*_into(writer)` twin that writes to any
`io.Writer`.
...
| WebP | lossless and lossy, with alpha; not animations | lossless and lossy, with alpha |
...
let blink = image.Animation { frames: .{ image.Frame { image: on, delay_ms: 500 }, image.Frame { image: off, delay_ms: 500 } } }
let gif = blink.encode_gif() ! panic("Cannot encode: " + E.message)
```

GIF holds up to 256 colours per frame. An image with more gets a palette chosen for it, with
dithering unless `dither` is false; pixels with alpha below 128 become transparent.
